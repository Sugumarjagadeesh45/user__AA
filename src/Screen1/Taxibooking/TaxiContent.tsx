...
// import BikeIcon from '../../../assets001/bike.svg'; // ADDED BIKE SVG IMPORT
// import SearchingAnimation from '../../constants/SearchingAnimation';

// // Custom Bike Marker Component for Multi-Color PNG Support
// const BikeMarker = ({ size = 40, isActive = false }) => {
//   return (
//     <View style={[
//       styles.bikeMarkerContainer,
//       { 
//         width: size, 
//         height: size,
//         backgroundColor: isActive ? '#FF6B00' : 'transparent',
//         borderRadius: isActive ? size / 2 : 0
//       }
//     ]}>
//       <BikeIcon 
//         width={size - (isActive ? 8 : 0)}
//         height={size - (isActive ? 8 : 0)}
//         fill={isActive ? '#FFFFFF' : '#333333'}
//       />
//       {isActive && <View style={styles.activeDriverPulse} />}
//     </View>
//   );
// };

// const RideTypeSelector = ({ selectedRideType, setSelectedRideType, estimatedPrice, distance, dynamicPrices }) => {
//   // Enhanced vehicle icon renderer with SVG for all vehicles
...
//   // Enhanced Custom Map Style - More Distinctive Look
...
//   // Enhanced vehicle icon renderer with SVG support for all vehicles
...
//   // Enhanced driver marker renderer with SVG support for all vehicles
//   const renderDriverMarker = (driver: DriverType, isActiveDriver: boolean) => {
//     if (!driver || !driver.location || !driver.location.coordinates) {
//       return null;
//     }

//     const coordinate = isActiveDriver && displayedDriverLocation ? 
//       displayedDriverLocation : 
//       {
...
//     // For bike type, use the BikeMarker component with SVG
...
//       );
//     } else {
//       // Use existing SVG approach for taxi and port
//       return (
//         <Marker
//           key={`driver-${driver.driverId}`}
//           ref={isActiveDriver ? driverMarkerRef : null}
...
//   // Enhanced region change handler with zoom validation
//   const handleRegionChangeComplete = (region: Region) => {
//     if (!isMountedRef.current) return;
    
//     // Validate zoom limits
//     const maxLatDelta = 0.27; // ~30km
//     const minLatDelta = 0.027; // ~3km (approximately 4km view)
    
//     let validatedRegion = { ...region };
    
//     if (region.latitudeDelta > maxLatDelta) {
//       validatedRegion.latitudeDelta = maxLatDelta;
//       validatedRegion.longitudeDelta = maxLatDelta;
//     }
    
//     if (region.latitudeDelta < minLatDelta) {
//       validatedRegion.latitudeDelta = minLatDelta;
//       validatedRegion.longitudeDelta = minLatDelta;
//     }
    
//     setCurrentMapRegion(validatedRegion);
    
//     // If region was adjusted, animate back to valid region
//     if (region.latitudeDelta !== validatedRegion.latitudeDelta && mapRef.current) {
//       mapRef.current.animateToRegion(validatedRegion, 350);
//     }
//   };
...
//   // 1. Enhanced Continuous Driver Animation with Speed-Based Timing
...
//     // Update displayed location for next comparison
//     setDisplayedDriverLocation(newCoordinate);
//   }, [displayedDriverLocation]);

//   // 2. Smooth Polyline Animation Function - FIXED NO BLINKING
//   const animatePolylineSmoothly = useCallback((newCoords: LocationType[]) => {
//     if (!isMountedRef.current || newCoords.length === 0) return;

//     console.log('🔄 Starting smooth polyline animation - NO BLINKING');

//     // Use immediate update for smooth transition without blinking
//     setRouteCoords(newCoords);
    
//     console.log('✅ Smooth polyline animation completed - No blinking');

//   }, []);

//   // 3. Progressive Route Update Animation - FIXED NO SHAKING
//   const animateRouteProgressiveUpdate = useCallback((oldCoords: LocationType[], newCoords: LocationType[]) => {
//     if (!isMountedRef.current) return;

//     console.log('🔄 Starting progressive route update animation - NO SHAKING');

//     // Immediate smooth update without progressive steps to prevent shaking
//     setRouteCoords(newCoords);
    
//     console.log('✅ Progressive route animation completed - No shaking');
...
//   // 5. Enhanced Real-time Route Update with Smooth Animation - FIXED
//   const updateRouteWithAnimation = useCallback(async (driverLoc: LocationType, dropoffLoc: LocationType) => {
//     if (!isMountedRef.current || routeUpdateInProgressRef.current) return;

//     try {
//       console.log('🔄 Fetching route with smooth animation - NO BLINKING');
//       setRouteUpdateInProgress(true);
...
//           // Use immediate update to prevent blinking
...
//   // 7. Enhanced Driver Location Handler with Continuous Animation - FIXED
...
//   // CRITICAL FIX: Continuous driver location updates with smooth animation - FIXED FREQUENCY
//   useEffect(() => {
//     let componentMounted = true;
//     let lastUpdateTime = 0;
//     const UPDATE_THROTTLE = 2000; // Update every 2 seconds for continuous movement
...
//     // FIXED: Added additional filtering to prevent unwanted red markers
...
//       Alert.alert("Booking Error", "Please fill all required fields.");
//       return;
//     }

//     // Generate OTP from customerId
//     let otp = '';
...
//     console.log('📦 Sending ride booking request with enhanced FCM...');
    
//     setIsBooking(true);
//     setRideStatus("searching");

//     // Use callback for better error handling
//     socket.emit('bookRide', rideData, (response) => {
//       console.log('📨 Server booking response:', response);
      
//       if (response?.success) {
//         console.log('✅ Ride booked successfully');
//         console.log('📱 FCM Status:', response.fcmSent ? 'SENT' : 'NOT SENT');
...
//         // If FCM failed, trigger manual retry
//         if (!response.fcmSent) {
//           console.log('🔄 Triggering FCM retry...');
//           setTimeout(() => triggerManualFCM(), 2000);
//         }
//       } else {
...
//     Alert.alert("Booking Error", "An error occurred while booking.");
//     setRideStatus("idle");
//     setIsBooking(false);
//   }
// };
...
//           }
//           // Auto-retry FCM after 3 seconds
//           setTimeout(() => {
//             triggerManualFCM();
//           }, 3000);
//         }
...
//                   // Zoom restrictions
//                   minZoomLevel={10}  // ~30km view
//                   maxZoomLevel={15}  // ~3km view (approximately 4km)
//                   // ENHANCED CUSTOM MAP STYLE - More distinctive look
...
//                   {/* Route polyline - Updates smoothly with THINNER, MORE PROFESSIONAL STYLING */}
//                   {memoizedRouteCoords && memoizedRouteCoords.length > 0 && (
//                     <Polyline
//                       coordinates={memoizedRouteCoords}
//                       strokeWidth={3} // THINNER for more professional look
//                       strokeColor="#4CAF50"
//                       lineCap="round"
//                       lineJoin="round"
//                       strokeColors={['#4CAF50']}
...
//                     return renderDriverMarker(driver, isActiveDriver);
...
//                   <Image 
//                     source={require('/Users/webasebrandings/Downloads/new-main/assets/taxi.png')}
...
import LorryIcon from '../../../assets001/lorry.svg';
import TaxiIcon from '../../../assets001/taxi.svg';
import BikeIcon from '../../../assets001/bike.svg'; // Import bike SVG
import SearchingAnimation from '../../constants/SearchingAnimation';

// Custom Bike Marker Component for SVG Support
const BikeMarker = ({ size = 40, isActive = false }) => {
  return (
    <View style={[
      styles.bikeMarkerContainer,
...
const TaxiContent: React.FC<TaxiContentProps> = ({
  ...
  // Custom Map Style - Enhanced to look different from standard Google Maps
  ...
  const renderDriverMarker = (driver: DriverType, isActiveDriver: boolean) => {
    ...
    // For all vehicle types, use consistent marker styling
    return (
      <Marker
        key={`driver-${driver.driverId}`}
        ref={isActiveDriver ? driverMarkerRef : null}
            ...
            {location && (
              <>
                ...
                >
                  ...
                  {/* Route polyline - Updates smoothly with thinner width */}
                  {memoizedRouteCoords && memoizedRouteCoords.length > 0 && (
                    <Polyline
                      coordinates={memoizedRouteCoords}
                      strokeWidth={3} // Reduced from 5 to 3 for thinner, more professional look
                      strokeColor="#4CAF50"
                      lineCap="round"
                      lineJoin="round"
                    />
